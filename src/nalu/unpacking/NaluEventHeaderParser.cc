...
NaluEventHeaderParser::NaluEventHeaderParser() : Parser() {
    ...
    time_threshold_data_location_ =       DataLocation(1, 24, 4);                   // Default
    clock_frequency_data_location_ =      DataLocation(1, 56, 1, 2, 0, 3);          // Default
    packet_size_data_location_ =          DataLocation(2, 24, 2);
    channel_mask_data_location_ =         DataLocation(2, 40, 3, 3, 0, 5);
    num_windows_data_location_ =          DataLocation(3, 40, 1);
    num_packets_data_location_ =          DataLocation(3, 48, 2);
    ...
    if (!unpacker_path) {
        ...
        if (!file) {
            utils::LoggerHolder::getInstance().WarningLogger 
                ...
                << ". Using default values for data location instead.\n";
        } else {
            json j;
            file >> j;
            ...
            time_threshold_data_location_ =      ParseJsonData(GetSection(nalu_event_header, "time_threshold"));
            clock_frequency_data_location_ =     ParseJsonData(GetSection(nalu_event_header, "clock_frequency"));
...
uint32_t NaluEventHeaderParser::GetTimeThreshold()       const { return GetData(time_threshold_data_location_); }
uint32_t NaluEventHeaderParser::GetClockFrequency()      const { return GetData(clock_frequency_data_location_); }
...
std::ostringstream NaluEventHeaderParser::Stream() {
    ...
    oss << "            TimeThreshold:      " << std::dec << GetTimeThreshold() << std::endl;
    oss << "            ClockFrequency:     " << std::dec << GetClockFrequency() << std::endl;
